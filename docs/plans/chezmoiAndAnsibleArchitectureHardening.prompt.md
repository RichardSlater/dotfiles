...
4. Refactor `dot_config/windows_aliases.tmpl` only after documenting the desired WSL user path behavior. Preferred direction: derive candidate Windows user paths from Chezmoi data or a small list in `.chezmoi.toml.tmpl`, rather than hard-coding `rslater`, `mcp`, and `ensono` in the template body.
